    ...
    class Functions : IScript
    {
        ...
        public void GiveLicense(IPlayer player, int targetCharId, string licShort)
        {
            try
            {
                ...
                if(ServerFactions.GetCharacterFactionId(charId) != 5) { HUDHandler.SendNotification(player, 3, 5000, "Du bist kein Angehöriger der Fahrschule."); return; }
                ...
                if(!CharactersLicenses.ExistServerLicense(licShort)) { HUDHandler.SendNotification(player, 3, 5000, "Ein unerwarteter Fehler ist aufgetreten."); return; }
